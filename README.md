...
**Access matches inventory admin:** any signed-in Supabase Auth user (the same accounts that manage trucks at `/admin`). For a small SKL team, create each person as an Auth user — they get inventory and sourcing (including “Run search now”) together.
...
2. Run **`supabase/sourcing-schema.sql`** in the Supabase SQL Editor (idempotent / safe to re-run) — updates `is_sourcing_staff()` to authenticated-admin access and keeps the search lock
...
6. Optionally set **`SOURCING_STAFF_EMAILS`** only if you want to narrow sourcing below full admin access
